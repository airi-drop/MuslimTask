...
type Props = {
  /** All unseen save dates (YYYY-MM-DD). Empty = nothing to show. */
  saveDates: string[];
  onDismiss: () => void;
};

/**
 * Stacked toasts for streak lives consumed. Follows the AchievementToast
 * pattern: renders one card per date, auto-dismisses after 6s, tap dismisses all.
 */
export function StreakSaveToast({ saveDates, onDismiss }: Props) {
  useEffect(() => {
    if (saveDates.length === 0) return;
    const t = setTimeout(onDismiss, 6000);
    return () => clearTimeout(t);
  }, [saveDates, onDismiss]);

  if (saveDates.length === 0) return null;
